...
// "null weight = equal" applies → plain average of all scored items (never drop one).
...
export function computeOverall(
  ...
  if (goalScore == null) return round2(competencyScore); // symmetric: only competency scored
