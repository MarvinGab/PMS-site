...
function saveWizardState(orgKey, payload) {
  ...
  persistWizardState(orgKey, payload);
}
...
function UploadValidationPanel({ title = 'Validation errors', errors = [], maxHeight = 220 }) {
  if (!Array.isArray(errors) || errors.length === 0) return null;

  const normalizeCode = (error) => String(error?.code || error?.field || 'UPLOAD').trim().toUpperCase().replace(/\s+/g, '_');
  const rowLabel = (error) => {
    if (typeof error?.row === 'number') return `Row ${error.row}`;
    if (error?.row !== undefined && error?.row !== null && error.row !== '') return String(error.row);
    if (error?.group) return 'GROUP';
    return '--';
  };
    ...
    <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 10, overflow: 'hidden' }}>
      <div style={{ padding: '10px 16px', background: '#FEE2E2', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: '#DC2626' }}>
          {errors.length} {title}{errors.length !== 1 ? 's' : ''} — fix and re-upload
        </span>
      </div>
      <div style={{ maxHeight, overflowY: 'auto' }}>
        {errors.map((err, i) => (
          <div key={`${normalizeCode(err)}_${i}`} style={{ display: 'grid', gridTemplateColumns: '60px 120px 1fr', gap: 8, padding: '7px 16px', borderTop: '1px solid #FECACA', fontSize: 12, color: '#374151' }}>
            <span style={{ color: '#9CA3AF', fontFamily: 'monospace' }}>{rowLabel(err)}</span>
            <span style={{ color: '#6B7280', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{normalizeCode(err)}</span>
            <span style={{ color: '#DC2626' }}>{err?.message || 'Upload row is invalid.'}</span>
          </div>
        ))}
      </div>
    </div>
...
function FrameworkProcessGraphic({ framework, applied, onApply }) {
  const stageMeta = {
    Perspectives: {
      icon: '◔',
  ...
  return (
    <div className="framework-process-graphic">
      ...
      <div className="framework-process-footer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        {applied ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 11px', borderRadius: 999, background: '#F0FDF4', border: '1px solid #86EFAC', color: '#15803D', fontSize: 11.5, fontWeight: 600 }}>
            ✓ Framework applied
          </span>
        ) : onApply ? (
          <button
            type="button"
            onClick={onApply}
            style={{ padding: '7px 16px', border: 'none', borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', background: '#2563EB', color: '#fff', fontFamily: 'inherit' }}
          >
            Apply framework
          </button>
        ) : null}
      ...
      <Card>
        ...
        <CardBody>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12, marginBottom: 16 }}>
            {FRAMEWORKS.map(fw => {
              const locked = fw.id === 'kra' || fw.id === 'custom';
              return (
                <button
                  type="button"
                  key={fw.id}
                  onClick={() => { if (!locked) update('frameworkId', fw.id); }}
                  disabled={locked}
                  ...
                  style={{
                    ...
                    background: locked ? '#F8FAFC' : (config.frameworkId === fw.id ? fw.color + '12' : '#fff'),
                  ...
                  {locked && (
                    <span style={{
                      ...
                      background: '#0F172A', color: '#fff', textTransform: 'uppercase',
                    }}>Coming soon</span>
                  )}
...
const PERSPECTIVE_COLORS = ['#3B82F6', '#38BDF8', '#6366F1', '#818CF8', '#A78BFA', '#22D3EE', '#8B5CF6'];
...
function GoalLibrarySheetControls({ config, downloadConfig = null, existingLibraries, onImported, onUploadErrors = null }) {
  const fileRef = useRef(null);
  const wrapRef = useRef(null);
  const [phase, setPhase] = useState('idle');
  const [message, setMessage] = useState('');
  ...
  async function handleUpload(event) {
    ...
    try {
      ...
      if (uploadErrors.length > 0) {
        setPhase('error');
        setMessage(`${uploadErrors.length} upload issue${uploadErrors.length === 1 ? '' : 's'} found.`);
        onUploadErrors?.(uploadErrors);
        return;
      }
      ...
      const nextErrors = formatUploadErrorMessages(err).map((message) => ({ field: 'parse', message }));
      setPhase('error');
      setMessage(nextErrors.map(error => error.message).join(' '));
      onUploadErrors?.(nextErrors);
    } finally {
      event.target.value = '';
    }
  }
...
function formatUploadErrorMessages(error) {
  return String(error?.message || 'Could not import the uploaded sheet.')
    .split(/\n+/)
    .map(message => message.trim())
    .filter(Boolean);
}

function getGroupGoalLibraryUploadConfig(config, group) {
  return {
    ...config,
    goalGroups: [group],
    goalKpiMode: group?.libraryType === 'kra-kpi' ? 'kra-kpi' : 'kra-only',
    goalLibraryScope: 'common',
    goalSegmentValues: [],
  };
}

function validateBulkGoalLibraryUpload(config, importedLibraries = []) {
  const groups = (config?.goalGroups || []).filter(isGroupLibraryEnabled);
  const groupLookup = new Map(groups.map(group => [String(group.name || '').trim().toLowerCase(), group]));
  const allSlots = groups.flatMap(group =>
    getGroupLibraryAssignments(group).map(assignment => ({
      group,
      assignment,
      groupKey: String(group.name || '').trim().toLowerCase(),
      labelKey: String(assignment.label || '').trim().toLowerCase(),
    }))
  );
  const slotLabels = allSlots.map(slot => slot.assignment.label).filter(Boolean);
  const errors = [];

  (importedLibraries || []).forEach((library) => {
    const libraryName = String(library?.name || '').trim();
    const libraryKey = libraryName.toLowerCase();
    const groupName = String(library?.groupName || '').trim();
    const groupKey = groupName.toLowerCase();
    let targetGroup = groupKey ? groupLookup.get(groupKey) : null;

    if (groupName && !targetGroup) {
      errors.push({
        field: 'group_name',
        message: `Group Name "${groupName}" is not configured for Goal Libraries. Use one of: ${groups.map(g => g.name).filter(Boolean).join(', ') || 'none configured'}.`,
      });
      return;
    }
    ...
    let slotMatch = null;
    if (targetGroup) {
      slotMatch = allSlots.find(slot => slot.group.id === targetGroup.id && slot.labelKey === libraryKey);
      if (!slotMatch) {
        const validForGroup = getGroupLibraryAssignments(targetGroup).map(assignment => assignment.label).filter(Boolean);
        errors.push({
          field: 'library_name',
          message: `Library Name "${libraryName || 'blank'}" is not valid for "${targetGroup.name}". Use one of: ${validForGroup.join(', ') || 'none configured'}.`,
        });
        return;
      }
    } else {
      const plainMatches = allSlots.filter(slot => slot.labelKey === libraryKey);
      if (plainMatches.length === 1) {
        slotMatch = plainMatches[0];
        targetGroup = slotMatch.group;
      } else if (plainMatches.length > 1) {
        errors.push({
          field: 'group_name',
          message: `Library Name "${libraryName}" exists in multiple groups. Add the exact Group Name column value to the sheet.`,
        });
        return;
      } else {
        errors.push({
          field: 'library_name',
          message: `Library Name "${libraryName || 'blank'}" is not configured. Use one of: ${slotLabels.join(', ') || 'none configured'}.`,
        });
        return;
      }
    }

    const validationConfig = getGroupGoalLibraryUploadConfig(config, targetGroup || slotMatch?.group);
    validateGoalLibraryData(
      { byAttr: false, data: flattenLibraryPerspectivesForValidation(library.perspectives || []) },
      validationConfig
    )
      .filter(error => error.field === 'perspective' || error.field === 'kra_weight' || error.field === 'kpi_weight')
      .forEach(error => {
        errors.push({
          ...error,
          message: `${targetGroup?.name || 'Goal Library'} / ${libraryName || 'Untitled'}: ${error.message}`,
        });
      });
  });
...
function PrefillSheetControls({ config, selectedGroup, onImported, phase, setPhase, errors, setErrors }) {
  ...
  async function handleUpload(event) {
    ...
    try {
      ...
      const expectedGroupName = String(selectedGroup.name || '').trim();
      const uploadMappingErrors = [];
      const importedLibraries = result.libraries || [];

      importedLibraries.forEach((library) => {
        const libraryName = String(library?.name || '').trim();
        const libraryGroupName = String(library?.groupName || '').trim();
        if (libraryGroupName && expectedGroupName && libraryGroupName.toLowerCase() !== expectedGroupName.toLowerCase()) {
          uploadMappingErrors.push({
            field: 'group_name',
            message: `Group Name "${libraryGroupName}" does not match the selected group "${expectedGroupName}".`,
          });
        }
        if (!libraryName || !expectedCardNamesLower.includes(libraryName.toLowerCase())) {
          uploadMappingErrors.push({
            field: 'card_name',
            message: `Card Name "${libraryName || 'blank'}" is not configured for ${expectedGroupName || 'this group'}. Use one of: ${expectedCardNames.join(', ') || 'none configured'}.`,
          });
        }
      });

      if (uploadMappingErrors.length > 0) {
        setErrors(uploadMappingErrors);
        setPhase('error');
        return;
      }

      const validationConfig = getGroupPrefillUploadConfig(config, selectedGroup);
      const validationErrors = importedLibraries.flatMap((library) => (
        validateGoalLibraryData(
          { byAttr: false, data: flattenLibraryPerspectivesForValidation(library.perspectives || []) },
          validationConfig
        ).map((error) => ({
      ...
      onImported(importedLibraries);
      setPhase('done');
      setMessage(`${result.count || 0} pre-fill card${(result.count || 0) === 1 ? '' : 's'} imported.`);
      setOpen(false);
    } catch (error) {
      const nextErrors = formatUploadErrorMessages(error).map((message) => ({ field: 'parse', message }));
...
function GroupStrategyCard({ group, index, totalGroups, onUpdate, onDelete, libraries = [], frameworkId = 'bsc' }) {
  ...
  const frameworkHasKpis = frameworkId !== 'kra';
              ...
              {[
                ...
                return (
                  <button
                    key={opt.id}
                    type="button"
                    onClick={() => onUpdate({ kpiRatingMode: opt.id })}
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: 8,
                      ...
                      cursor: 'pointer',
                      fontFamily: 'inherit',
                      border: `1px solid ${active ? '#EC4899' : '#FBCFE8'}`,
                      background: active ? '#FFFFFF' : 'rgba(255,255,255,0.6)',
                      color: active ? '#BE185D' : '#475569',
                      boxShadow: active ? '0 1px 3px rgba(236,72,153,0.2)' : 'none',
                      minHeight: 36,
                      transition: 'all .15s',
                    }}
                  >
                    <span style={{
                      ...
                      height: 18,
                      borderRadius: '50%',
                      border: `1.5px solid ${active ? '#EC4899' : '#CBD5E1'}`,
                    ...
                    <span style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: 11.5, color: active ? '#EC4899' : '#94A3B8', fontWeight: 500 }}>{opt.hint}</span>
                    </span>
                  </button>
                );
              })}
...
function StepGroups({ config, update }) {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto', paddingBottom: 40 }}>
      <SectionHead title="Who gets what goal approach?" />
...
function getLibraryWeightStats(library, { showKpiPercent = false } = {}) {
  const perspectives = library?.perspectives || [];
  const kraWeight = (kra) => Number(kra?.suggestedWeight ?? kra?.weight ?? 0) || 0;
  const kpiWeight = (kpi) => Number(kpi?.suggestedWeight ?? kpi?.weight ?? 0) || 0;
  const perspectivesWithKras = perspectives.filter((p) => (p.kras || []).length > 0);
  let kraPct = null;
  if (perspectivesWithKras.length > 0) {
    const sums = perspectivesWithKras.map((p) =>
      (p.kras || []).reduce((sum, kra) => sum + kraWeight(kra), 0)
    );
    const avg = sums.reduce((a, b) => a + b, 0) / sums.length;
    kraPct = Math.round(avg);
  }
  let kpiPct = null;
  if (showKpiPercent) {
    const krasWithKpis = perspectives.flatMap((p) => (p.kras || []).filter((kra) => (kra.kpis || []).length > 0));
    if (krasWithKpis.length > 0) {
      const sums = krasWithKpis.map((kra) =>
        (kra.kpis || []).reduce((sum, kpi) => sum + kpiWeight(kpi), 0)
      );
      const avg = sums.reduce((a, b) => a + b, 0) / sums.length;
      kpiPct = Math.round(avg);
    }
  }
  return { kraPct, kpiPct };
}

function WeightPctBadge({ label, pct }) {
  if (pct == null) return null;
  const ok = pct === 100;
  return (
    <span
      title={`${label} weights total ${pct}%${ok ? '' : ' — should be 100%'}`}
      style={{
        fontSize: 10.5,
        fontWeight: 700,
        padding: '1px 8px',
...
function LibraryCard({ library, groupLabel, assignedText, accent, active = false, onSelect, onEdit, onDelete, selectionHint, empty = false, warning = null, showKpiPercent = false, setupLabels = [] }) {
  ...
  const { kraPct, kpiPct } = getLibraryWeightStats(library, { showKpiPercent });
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              ...
              <WeightPctBadge label="KRA" pct={kraPct} />
              <WeightPctBadge label="KPI" pct={kpiPct} />
            </div>
          </div>
          {onDelete ? (
            <button
...
function getGoalSetupLabels(group = {}, { includePrefill = false } = {}) {
  const prefillLabel = !includePrefill ? null : group.prefillType === 'kra-kpi'
    ? 'Pre-fill KRAs + KPIs'
                        ...
                        {(kra.kpis || []).map(kpi => (
                          <div key={kpi.id} style={{ marginBottom: 8 }}>
                            <div style={{ display: 'flex', gap: 8, marginBottom: 4, alignItems: 'center' }}>
                              ...
                              {weightType !== 'none' && !hideKpiWeights && (
                                <>
                                  <input
                                    type="text"
                                    inputMode="decimal"
...
export function StepGoalLibraries({ config, update, hideHead, readOnly = false, showPrefillGroups = false }) {
  const libraries = config.goalLibraries || [];
  const groups = config.goalGroups || [];
  const usesBscPerspectives = config?.frameworkId === 'bsc';
  const groupsNeedingLib = groups.filter(showPrefillGroups ? isGroupShownOnGoalsBoard : isGroupLibraryEnabled);
  ...
  const allSlotsAssigned = groupsWithAssignments.length === 0 || groupsWithAssignments.every(group =>
    !isGroupLibraryEnabled(group) || group.libraryAssignments.every(assignment => assignment.libraryId && libraries.some(library => library.id === assignment.libraryId))
  );
  ...
  const editingPrefillGroup = groupsWithAssignments.find(group => group.id === editingPrefillTarget?.groupId) || null;
  const editingPrefillAssignment = editingPrefillGroup
    ? getGroupPrefillAssignments(editingPrefillGroup).find(assignment => assignment.slotKey === editingPrefillTarget?.slotKey) || null
    : null;
  ...
  return (
    <div style={{ maxWidth: 1180, margin: '0 auto', paddingBottom: 40 }}>
      ...
      {!readOnly && showPrefillGroups && editingPrefillGroup && editingPrefillAssignment && (
        <AddLibraryModal
          config={getGroupPrefillUploadConfig(config, editingPrefillGroup)}
          initialLibrary={{
            id: `${editingPrefillGroup.id}_${editingPrefillAssignment.slotKey}`,
                ...
                <div key={group.id}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 18 }}>
                    {slots.map((slot, index) => {
                      ...
                      const prefillOnly = showPrefillGroups && !libraryEnabled && !!group.prefillType;
                      const displayLibrary = library || (prefillOnly
                        ? { name: assignment.label, type: group.prefillType === 'kra-only' ? 'kra-only' : 'kra-kpi', perspectives: prefillData }
                        : { name: assignment.label, type: group.libraryType === 'kra-only' ? 'kra-only' : 'kra-kpi', perspectives: [] });
                      const hasData = libraryEnabled ? !!library : prefillData.length > 0;
                      return (
                        <LibraryCard
                          key={`${group.id}:${assignment.slotKey}`}
                          ...
                          setupLabels={!readOnly ? getGoalSetupLabels(group, { includePrefill: showPrefillGroups }) : []}
                          onSelect={readOnly && hasData ? () => setViewingLibrary(displayLibrary) : null}
                          ...
                          selectionHint={readOnly
                            ? (hasData ? 'Click to view details' : libraryEnabled ? 'No library configured' : 'No pre-fill data configured')
                            : (hasData
                                ? `${group.name}${group.segmentAttr ? ` · ${group.segmentAttr}` : ''}`
                                : `${libraryEnabled ? 'Create library' : 'Create pre-fill data'} for ${assignment.label}`)}
                        />
                      );
                    })}
                  </div>
      ...
      <UploadValidationPanel title="validation error" errors={uploadErrors} />
        ...
        pendingMessage={
          groupsWithAssignments.length === 0
            ...
            : uploadErrors.length > 0
              ? 'Fix the goal library upload errors shown above, then re-upload.'
            : !allSlotsAssigned
        ...
        invalidMessage={
          uploadErrors.length > 0
            ...
            : !allSlotsAssigned
              ? 'Open each card and configure a library for every attribute value.'
              : mismatchedLibraries.length > 0
                ? `Fix perspective mismatch on ${mismatchedLibraries.length === 1 ? 'library' : 'libraries'}: ${mismatchedLibraries.map(l => l.name || 'Untitled').join(', ')}.`
                : 'Fix the KRA library issues above before confirming.'
        }
...
function StepLimits({ config, update }) {
  ...
  return (
    <div style={{ maxWidth: 820, margin: '0 auto', paddingBottom: 40 }}>
      <SectionHead title="KRA/KPI count rules" />
...
function StepSummary({ config, onLaunched }) {
  ...
  const coverageByGroup = groupSummary.map((group) => {
    ...
    members.forEach((employee) => {
      ...
      // Only track direct-reports inside this group. Managers outside the group
      // (or outside the PMS entirely) are treated like any other reference —
      // we don't render them as placeholder nodes.
      if (!memberCodeMap.has(managerCode)) return;
    ...
    // A root node is any employee whose reporting manager is outside this
    // group's member set (no manager at all, or manager referenced outside PMS
    // / in another group). We don't create placeholder cards — these folks
    // simply render as top-level nodes.
  ...
  return (
    <div style={{ maxWidth: SUMMARY_MAX_WIDTH, margin: '0 auto', paddingBottom: 60 }}>
      <SectionHead title="Review & Launch" />
...
function StepGoalLibrary({ config, update }) {
  ...
  function applyConfig() {
    ...
    update('goalEmployeeEdit', 'edit-freely');
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', paddingBottom: 40 }}>
      <SectionHead title="Goal Creation Strategy" />
...
function GroupDataUploadPanel({ group, config, update }) {
  ...
  async function handleImport(type, e) {
    ...
    try {
      ...
      setImportErrors(prev => ({ ...prev, [type]: formatUploadErrorMessages(err).map((message) => ({ field: 'parse', message })) }));
      setImportPhase(prev => ({ ...prev, [type]: 'error' }));
    }
  }
...
function StepKRALibrary({ config, update }) {
  ...
  if (usingGroups) {
    return (
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <SectionHead title="Upload KRA / Library Data" />
  ...
  async function handleImport(e) {
    ...
    try {
      ...
      setImportErrors(formatUploadErrorMessages(err).map((message) => ({ group: null, kraName: null, field: 'parse', message })));
      setImportPhase('error');
    }
  }
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      if (uploadedEmployees.length > 0) {
        // Each employee now gets a unique 6-digit OTP at credential creation
        // time, generated inside syncEmployeeCredentialsForOrg. The org-wide
        // password no longer gates this step.
          ...
          {uploadState && uploadState.status !== 'parsing' && (
            <div style={{ marginTop: 14 }}>
              ...
              {uploadState.status === 'invalid' && uploadState.errors?.length > 0 && (
                <UploadValidationPanel title="validation error" errors={uploadState.errors} />
              )}
...
/* ═══════════════════════════════════════════════════════════════════════════
   KRA/KPI FLAT COMPONENTS — parallel to BSC, no perspective concept.
   The data shape still uses a single implicit "All KRAs" perspective so
   downstream helpers stay framework-agnostic, but the UI exposes a flat
   KRA list with KPIs nested under each KRA.
══════════════════════════════════════════════════════════════════════════ */

const FLAT_IMPLICIT_PERSP_NAME = 'All KRAs';

function flatPerspectiveFromKras(kras) {
  return [{
    id: `lp_flat_${Date.now()}_${Math.random().toString(36).slice(2, 5)}`,
    name: FLAT_IMPLICIT_PERSP_NAME,
    weight: 100,
    kras: kras || [],
  }];
}

function krasFromFlatPerspective(perspectives) {
...
function AddLibraryModalFlat({ initialLibrary = null, fixedName = '', suggestedType = null, lockType = false, hideKpiWeights = false, targetsOn = false, targetLevel = null, targetTypeOptions = [], onSave, onClose }) {
  ...
  const [type, setType] = useState(normalizedInitialLibrary?.type || suggestedType || 'kra-only');
  const [weightType, setWeightType] = useState(normalizedInitialLibrary?.weightType || 'suggested');
  const [kras, setKras] = useState(seedKras);
  const [openKraDescriptions, setOpenKraDescriptions] = useState({});

  useEffect(() => {
    setKras(prev => prev.map(k => ({
      ...k,
      suggestedWeight: toNonNegativeWeight(k.suggestedWeight),
      kpis: (k.kpis || []).map(kpi => ({ ...kpi, weight: toNonNegativeWeight(kpi.weight) })),
    })));
  ...
  function changeType(newType) {
    if (newType === type) return;
    const hasKpis = kras.some(k => (k.kpis || []).length > 0);
    if (newType === 'kra-only' && hasKpis) {
      if (!window.confirm('Switching to KRAs only will remove all KPI entries. Continue?')) return;
      setKras(current => current.map(k => ({ ...k, kpis: [] })));
    }
    setType(newType);
  }
  ...
  function updateKRA(kraId, changes) {
    setKras(current => current.map(k => k.id === kraId ? { ...k, ...changes } : k));
  }
  function removeKRA(kraId) {
    setKras(current => current.filter(k => k.id !== kraId));
    setOpenKraDescriptions(current => { const next = { ...current }; delete next[kraId]; return next; });
  }
  function updateKraWeight(kraId, rawValue) {
    const sanitized = sanitizeWeightInput(rawValue);
    setKras(current => current.map(k => k.id === kraId ? { ...k, suggestedWeight: sanitized === '' ? 0 : Number(sanitized) || 0 } : k));
  }
  function addKPI(kraId) {
    setKras(current => current.map(k => k.id !== kraId ? k : {
      ...k,
      kpis: [...(k.kpis || []), { id: `kpi_${Date.now()}_${Math.random().toString(36).slice(2, 5)}`, name: '', weight: 0 }],
    }));
  }
  function updateKPI(kraId, kpiId, changes) {
    setKras(current => current.map(k => k.id !== kraId ? k : {
      ...k,
      kpis: (k.kpis || []).map(kp => kp.id === kpiId ? { ...kp, ...changes } : kp),
    }));
  ...
  function updateKpiWeight(kraId, kpiId, rawValue) {
    const sanitized = sanitizeWeightInput(rawValue);
    setKras(current => current.map(k => k.id !== kraId ? k : {
      ...k,
      kpis: (k.kpis || []).map(kp => kp.id === kpiId ? { ...kp, weight: sanitized === '' ? 0 : Number(sanitized) || 0 } : kp),
  ...
  const globalKraTotal = kras.reduce((s, k) => s + toNonNegativeWeight(k.suggestedWeight), 0);
  const resolvedName = fixedName || name;
  const canSave = resolvedName.trim() && (initialLibrary ? true : kras.some(k => k.name.trim()));
  const inputStyle = { border: '1px solid #E2E8F0', borderRadius: 6, padding: '6px 10px', fontSize: 13, outline: 'none', fontFamily: 'inherit', background: '#fff' };
  const textareaStyle = { ...inputStyle, width: '100%', boxSizing: 'border-box', resize: 'vertical', minHeight: 64, lineHeight: 1.45 };
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 760, boxShadow: '0 20px 60px rgba(0,0,0,.18)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '24px', maxHeight: '70vh', overflowY: 'auto' }}>
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 11.5, fontWeight: 600, color: '#64748B', display: 'block', marginBottom: 5 }}>Library Name *</label>
            {fixedName ? (
              <div style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', background: '#F8FAFC', color: '#334155', fontWeight: 700 }}>{fixedName}</div>
            ) : (
              <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Engineering Goals 2025" style={{ ...inputStyle, width: '100%', boxSizing: 'border-box' }} />
            )}
          </div>
          ...
          <div style={{ border: '1.5px solid #E2E8F0', borderRadius: 10, padding: '10px 14px' }}>
            {kras.map(kra => {
              const descOpen = !!openKraDescriptions[kra.id];
              const hasDesc = !!String(kra.desc || '').trim();
              return (
                <div key={kra.id} style={{ marginBottom: 8, border: '1px solid #F1F5F9', borderRadius: 8, padding: '10px 12px' }}>
                  <div style={{ display: 'flex', gap: 8, marginBottom: type === 'kra-kpi' ? 6 : 0 }}>
                    <input value={kra.name} onChange={e => updateKRA(kra.id, { name: e.target.value })} placeholder="KRA name" style={{ ...inputStyle, flex: 1 }} />
                    {weightType !== 'none' && (
                      <>
                        <input
                          type="text"
                          inputMode="decimal"
                          value={toNonNegativeWeight(kra.suggestedWeight) || ''}
                          onChange={e => updateKraWeight(kra.id, e.target.value)}
                          style={{ ...inputStyle, width: 60, textAlign: 'center' }}
                          placeholder="%"
                        />
                        <span style={{ fontSize: 12, color: '#9CA3AF', alignSelf: 'center' }}>%</span>
                  ...
                  <div style={{ marginBottom: type === 'kra-kpi' ? 8 : 0 }}>
                    <button
                      type="button"
                      onClick={() => setOpenKraDescriptions(current => ({ ...current, [kra.id]: !descOpen }))}
                      style={{ background: 'none', border: 'none', padding: 0, marginBottom: descOpen ? 6 : 0, color: hasDesc ? '#2563EB' : '#94A3B8', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                    >
                      {descOpen ? 'Hide KRA description' : hasDesc ? 'Edit KRA description' : '+ Add KRA description'}
                    </button>
                    {descOpen && (
                      <textarea
                        value={kra.desc || ''}
                        onChange={e => updateKRA(kra.id, { desc: e.target.value })}
                        placeholder="KRA description (optional)"
                        style={textareaStyle}
                      />
                    )}
                    ...
                    <div style={{ paddingLeft: 16 }}>
                      {(kra.kpis || []).map(kpi => (
                        <div key={kpi.id} style={{ marginBottom: 8 }}>
                          <div style={{ display: 'flex', gap: 8, marginBottom: 4, alignItems: 'center' }}>
                            <div style={{ width: 4, height: 4, borderRadius: '50%', background: '#CBD5E1', flexShrink: 0 }} />
                            <input value={kpi.name} onChange={e => updateKPI(kra.id, kpi.id, { name: e.target.value })} placeholder="KPI name" style={{ ...inputStyle, flex: 1, fontSize: 12 }} />
                            {weightType !== 'none' && !hideKpiWeights && (
                              <>
                                <input
                                  type="text"
                                  inputMode="decimal"
                                  value={toNonNegativeWeight(kpi.weight) || ''}
                                  onChange={e => updateKpiWeight(kra.id, kpi.id, e.target.value)}
                                  style={{ ...inputStyle, width: 55, textAlign: 'center', fontSize: 12 }}
                                  placeholder="%"
                                />
                                <span style={{ fontSize: 12, color: '#9CA3AF' }}>%</span>
            ...
            <button type="button" onClick={addKRA} style={{ fontSize: 12.5, color: '#2563EB', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: '4px 0', fontFamily: 'inherit' }}>+ Add KRA</button>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid #E9EDF2', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button type="button" onClick={onClose} style={{ padding: '9px 20px', border: '1px solid #E2E8F0', borderRadius: 8, background: '#fff', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
          <button type="button" disabled={!canSave} onClick={() => {
            const clampedKras = kras.map(k => ({
              ...k,
              suggestedWeight: weightType === 'none' ? 0 : Math.max(0, Number(k.suggestedWeight) || 0),
              kpis: (k.kpis || []).map(kpi => ({ ...kpi, weight: weightType === 'none' ? 0 : Math.max(0, Number(kpi.weight) || 0) })),
            }));
            onSave({
              id: initialLibrary?.id || `lib_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
              name: resolvedName.trim(),
              type,
              weightType,
              perspectives: flatPerspectiveFromKras(clampedKras),
            });
          }} style={{ padding: '9px 22px', background: canSave ? '#2563EB' : '#CBD5E1', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: canSave ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}>
            {initialLibrary ? 'Save Changes' : 'Save Library'}
          </button>
...
export function StepPrefillDataFlat({ config, update, readOnly = false }) {
  ...
  const selectedGroup = groupsNeedingPrefill.find(g => g.id === selectedGroupId) || groupsNeedingPrefill[0] || null;
  const editingPrefillGroup = groupsNeedingPrefill.find(g => g.id === editingPrefillTarget?.groupId) || null;
  const editingPrefillAssignment = editingPrefillGroup
  ...
  const boardSlots = ((showAllGroups && canShowAllGroups) ? groupsNeedingPrefill : (selectedGroup ? [selectedGroup] : []))
    .flatMap((group, groupIndex) =>
      getGroupPrefillAssignments(group).map((assignment, assignmentIndex) => ({
        group, assignment, accent: LIBRARY_BOARD_COLORS[(groupIndex + assignmentIndex) % LIBRARY_BOARD_COLORS.length],
      }))
    );
  ...
  function importPrefillLibraries(importedLibraries) {
    if (!selectedGroup) return;
    const normalizeLookup = v => String(v || '').trim().toLowerCase();
    const importedMap = new Map((importedLibraries || []).map(l => [normalizeLookup(l.name), l]));
    const nextAssignments = getGroupPrefillAssignments(selectedGroup).map(a => {
      const match = importedMap.get(normalizeLookup(a.label));
      return { ...a, data: match?.perspectives || [] };
    });
    updatePrefillAssignments(selectedGroup.id, nextAssignments);
  }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {groupsNeedingPrefill.map((group, index) => {
                ...
                return (
                  <button key={group.id} type="button"
                    onClick={() => { setShowAllGroups(false); setSelectedGroupId(group.id); }}
                    style={{ borderRadius: 16, border: `1px solid ${active ? '#93C5FD' : '#E2E8F0'}`, background: active ? '#EFF6FF' : '#FFFFFF', color: active ? '#2563EB' : '#334155', padding: '12px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: active ? '0 10px 22px rgba(37,99,235,.08)' : 'none' }}>
                    {group.name || `Group ${index + 1}`}
                  </button>
                );
              })}
              {canShowAllGroups && (
                <button type="button" onClick={() => setShowAllGroups(true)}
                  style={{ borderRadius: 16, border: `1px solid ${showAllGroups ? '#93C5FD' : '#E2E8F0'}`, background: showAllGroups ? '#EFF6FF' : '#FFFFFF', color: showAllGroups ? '#2563EB' : '#334155', padding: '12px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: showAllGroups ? '0 10px 22px rgba(37,99,235,.08)' : 'none' }}>
                  All Employees
                </button>
              )}
            </div>
...
export function StepGoalLibrariesFlat({ config, update, hideHead, readOnly = false, showPrefillGroups = false }) {
  const libraries = config.goalLibraries || [];
  const groups = config.goalGroups || [];
  const groupsNeedingLib = groups.filter(showPrefillGroups ? isGroupShownOnGoalsBoard : isGroupLibraryEnabled);
  const canShowAllGroups = groupsNeedingLib.length > 1;
  ...
  useEffect(() => {
    if (showAllGroups && canShowAllGroups) return;
    if (!groupsWithAssignments.some(g => g.id === selectedGroupId)) {
      setSelectedGroupId(groupsWithAssignments[0]?.id || '');
    }
  }, [groupsWithAssignments, selectedGroupId, showAllGroups, canShowAllGroups]);

  const selectedGroup = groupsWithAssignments.find(g => g.id === selectedGroupId) || groupsWithAssignments[0] || null;
  const templateScopeConfig = (showAllGroups && canShowAllGroups) || !selectedGroup
    ? config
    : { ...config, goalGroups: [selectedGroup] };
  ...
  function saveLibrary(lib) {
    const exists = libraries.some(e => e.id === lib.id);
    const nextLibraries = exists ? libraries.map(e => e.id === lib.id ? lib : e) : [...libraries, lib];
    update('goalLibraries', nextLibraries);
    if (editingSlot) {
      const currentAssignments = getGroupLibraryAssignments(editingSlot.group);
      const nextAssignments = currentAssignments.map(a =>
        a.slotKey === editingSlot.assignment.slotKey ? { ...a, libraryId: lib.id } : a
      );
      update('goalGroups', setGroupLibraryAssignments(groups, editingSlot.group.id, nextAssignments));
    }
    update('goalLibrariesAppliedSnapshot', null);
  ...
  function deleteLibrary(id) {
    ...
    update('goalGroups', groups.map(group => ({
      ...group,
      libraryId: group.libraryId === id ? null : group.libraryId,
      libraryAssignments: getGroupLibraryAssignments(group).map(a =>
        a.libraryId === id ? { ...a, libraryId: null } : a
      ),
    })));
    update('goalLibrariesAppliedSnapshot', null);
    setUploadErrors([]);
  ...
  function handleImportedLibraries(nextLibraries, importedLibraries = nextLibraries) {
    ...
    const libraryById = new Map(nextLibraries.map(l => [l.id, l]));
    const librariesByName = new Map();
    const librariesByGroup = new Map();
    const librariesByCompositeKey = new Map();
    nextLibraries.forEach(l => {
      const nameKey = normalizeLookup(l.name);
      const groupKey = normalizeLookup(l.groupName);
      appendLookup(librariesByName, nameKey, l);
      appendLookup(librariesByGroup, groupKey, l);
      if (nameKey && groupKey) appendLookup(librariesByCompositeKey, `${groupKey}::${nameKey}`, l);
    });
    const assignedLibraryIds = new Set();
    const resolveAssignmentLibraryId = (group, assignment) => {
      if (assignment.libraryId && libraryById.has(assignment.libraryId)) return assignment.libraryId;
      const groupKey = normalizeLookup(group.name);
      const labelKey = normalizeLookup(assignment.label);
      const compositeMatches = groupKey && labelKey ? (librariesByCompositeKey.get(`${groupKey}::${labelKey}`) || []) : [];
      if (compositeMatches.length === 1) return compositeMatches[0].id;
      const plainMatches = labelKey ? (librariesByName.get(labelKey) || []) : [];
      if (plainMatches.length === 1) return plainMatches[0].id;
      const groupMatches = groupKey ? (librariesByGroup.get(groupKey) || []) : [];
      if (group.libraryAssignments.length === 1 && groupMatches.length === 1) return groupMatches[0].id;
      return null;
    };
    groupsWithAssignments.forEach(group => {
      const nextAssignments = group.libraryAssignments.map(a => {
        const libraryId = resolveAssignmentLibraryId(group, a);
        if (libraryId) assignedLibraryIds.add(libraryId);
        return { ...a, libraryId };
      });
      nextGroups = setGroupLibraryAssignments(nextGroups, group.id, nextAssignments);
    ...
    update('goalGroups', nextGroups);
    update('goalLibrariesAppliedSnapshot', null);
    const unassignedImportedLibraries = importedLibraries.filter(l => !assignedLibraryIds.has(l.id));
    return {
      unassignedImportCount: unassignedImportedLibraries.length,
      unassignedImportNames: unassignedImportedLibraries.map(l => {
        const groupLabel = String(l.groupName || '').trim();
        const libraryLabel = String(l.name || '').trim() || 'Untitled library';
        return groupLabel ? `${groupLabel} / ${libraryLabel}` : libraryLabel;
      }),
    };
  }
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 18 }}>
                    {slots.map((slot, index) => {
                      ...
                      const displayLibrary = library || (prefillOnly
                        ? { name: assignment.label, type: group.prefillType === 'kra-only' ? 'kra-only' : 'kra-kpi', perspectives: prefillData }
                        : { name: assignment.label, type: group.libraryType === 'kra-only' ? 'kra-only' : 'kra-kpi', perspectives: [] });
                      const libraryKraCount = (displayLibrary.perspectives || []).reduce((sum, p) => sum + (p.kras || []).length, 0);
                      const libraryLabel = displayLibrary.type === 'kra-kpi' ? '📊 KRAs + KPIs' : '📌 KRAs only';
                      const hasData = libraryEnabled ? !!library : prefillData.length > 0;
                      return (
                        <LibraryCard
                          key={`${group.id}:${assignment.slotKey}`}
                          ...
                          showKpiPercent={displayLibrary?.type === 'kra-kpi' && group.kpiRatingMode !== 'free-text'}
                          selectionHint={readOnly
                            ? (hasData ? `${libraryKraCount} KRA${libraryKraCount !== 1 ? 's' : ''} configured · click to view` : libraryEnabled ? 'No library configured' : 'No pre-fill data configured')
                            : (hasData
                                ? `${libraryKraCount} KRA${libraryKraCount !== 1 ? 's' : ''} · ${libraryLabel}`
                                : `${libraryEnabled ? 'Create or upload' : 'Create or upload pre-fill'} ${libraryLabel.toLowerCase()} for ${assignment.label}`)}
                        />
                      );
                    })}
                  </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {groupsWithAssignments.map((group, index) => {
                ...
                return (
                  <button key={group.id} type="button"
                    onClick={() => { setShowAllGroups(false); setSelectedGroupId(group.id); }}
                    style={{ borderRadius: 16, border: `1px solid ${active ? '#FCA5A5' : '#E2E8F0'}`, background: active ? '#FFF5F5' : '#FFFFFF', color: active ? '#DC2626' : '#334155', padding: '12px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: active ? '0 10px 22px rgba(248,113,113,.08)' : 'none' }}>
                    {group.name || `Group ${index + 1}`}
                  </button>
                );
              })}
...
function StepLimitsFlat({ config, update }) {
  const enabled = !!config.limitsEnabled;
  const groups = config.goalGroups || [];
  const rules = config.limitsRules || [];
  const hasKPIs = groups.some(g => g.mode === 'prefill') || (config.goalLibraries || []).some(l => l.type === 'kra-kpi');
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #F1F5F9' }}>
                <button type="button"
                  onClick={() => setAdvancedOpenByGroup(current => ({ ...current, [group.id]: !current[group.id] }))}
                  aria-expanded={advancedOpen}
                  style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, border: '1px solid #E2E8F0', borderRadius: 10, background: advancedOpen ? '#F8FAFC' : '#FFFFFF', padding: '11px 12px', cursor: 'pointer', textAlign: 'left' }}>
                  <div>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: '#0F172A' }}>Advanced rules</div>
                    <div style={{ fontSize: 11.5, color: '#6B7280', marginTop: 2 }}>Weight % limits for edge-case setups.</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                    <span style={{ fontSize: 11.5, fontWeight: 600, color: advancedValueCount > 0 ? '#2563EB' : '#94A3B8', background: advancedValueCount > 0 ? '#EFF6FF' : '#F8FAFC', borderRadius: 999, padding: '4px 9px' }}>
                      {advancedValueCount > 0 ? `${advancedValueCount} active` : 'Optional'}
                    </span>
                    <span style={{ fontSize: 15, color: '#64748B' }}>{advancedOpen ? 'Hide' : 'Show'}</span>
                  </div>
...
function isStepComplete(stepId, config) {
  switch (stepId) {
    ...
    case 'limits':
      if (config.frameworkId === 'bsc' || config.frameworkId === 'kra-kpi') {
...
function computeSetupProgress(config, visitedIterable) {
...
export default function PMSWizard({ onLaunched, orgKeyOverride, orgNameOverride }) {
  ...
  function saveAndExitSetup() {
    saveWizardState(workspace.orgKey, {
      ...
      config,
      visited: [...visited],
      setupProgress: computeSetupProgress(config, visited),
    });
    exitToLogin();
  }
  ...
  // Compute the actual destination Next would advance to.
  // - If the adjacent pair sibling is incomplete, route sideways to it.
  // - If the immediate next step is the complete sibling, skip past it.
  // - Otherwise advance one step.
  function getNextTargetIndex() {
    if (!isStepComplete(currentStepId, config)) return -1;
    if (pendingSiblingStep && pendingSiblingIndex >= 0) {
      return pendingSiblingIndex;
    }
    const siblingId = ADJACENT_PAIR[currentStepId];
    if (siblingId) {
      const siblingNeighbour = navSteps[step + 1];
      if (siblingNeighbour?.id === siblingId && isStepComplete(siblingId, config)) {
        return step + 2 < totalSteps ? step + 2 : -1;
      }
    }
    return step + 1 < totalSteps ? step + 1 : -1;
  }
  ...
  function next() {
    const target = getNextTargetIndex();
    if (target < 0) return;
  ...
  const stepComponents = navSteps.map((navStep) => {
    switch (navStep.id) {
      ...
      case 'prefill_data':
        return config.frameworkId === 'kra-kpi'
          ? <StepPrefillDataFlat key="prefill_data" config={config} update={update} />
          : <StepPrefillData key="prefill_data" config={config} update={update} />;
      case 'goal_libraries':
        return config.frameworkId === 'kra-kpi'
          ? <StepGoalLibrariesFlat key="goal_libraries" config={config} update={update} />
          : <StepGoalLibraries key="goal_libraries" config={config} update={update} />;
      case 'limits':
        if (config.frameworkId === 'bsc') return <StepLimits key="limits" config={config} update={update} />;
        if (config.frameworkId === 'kra-kpi') return <StepLimitsFlat key="limits" config={config} update={update} />;
        return <StepLimitsRules key="limits" config={config} update={update} />;
      case 'emp_settings':
  ...
  // Adjacent goal-data pair: prefill_data <-> goal_libraries can be tackled in either order,
  // so when one is complete and its sibling is not, Next routes sideways to the sibling
  // instead of advancing past the pair.
  const ADJACENT_PAIR = { prefill_data: 'goal_libraries', goal_libraries: 'prefill_data' };
  const pendingSiblingId = ADJACENT_PAIR[currentStepId];
  const pendingSiblingIndex = pendingSiblingId ? navSteps.findIndex(s => s.id === pendingSiblingId) : -1;
  const pendingSiblingStep = pendingSiblingIndex >= 0 && !isStepComplete(pendingSiblingId, config)
    ? navSteps[pendingSiblingIndex]
    : null;
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Geist','Inter','Segoe UI',Arial,sans-serif", fontSize: 14, color: '#0D1117', background: '#F8FAFC' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* PAIR PENDING STRIP — only useful when the pending sibling is BEHIND the
            user (they jumped ahead and the earlier step still needs completion).
            When the sibling is the natural next step, the Next button already
            takes them there, so this strip would just be noise. */}
        {canProceed && pendingSiblingStep && pendingSiblingIndex < safeStep && (
          <div style={{ padding: '9px 32px', background: '#EFF6FF', borderTop: '1.5px solid #BFDBFE', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 12.5, color: '#1E40AF', fontWeight: 500, flex: 1 }}>
              ↺ <strong>{pendingSiblingStep.label}</strong> still needs to be completed — Next will take you there before advancing to {navSteps[Math.max(pendingSiblingIndex, safeStep) + 1]?.label || 'the next step'}.
            </span>
            <button onClick={() => setStep(pendingSiblingIndex)} style={{ background: '#fff', border: '1px solid #93C5FD', color: '#2563EB', borderRadius: 7, padding: '4px 10px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              Go to {pendingSiblingStep.label} →
            </button>
          </div>
        )}
        ...
        <div style={{ padding: '14px 32px', background: '#fff', borderTop: '1.5px solid #E9EDF2', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', bottom: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button
              onClick={safeStep === totalSteps - 1 ? onLaunched : next}
              disabled={safeStep === totalSteps - 1 ? false : !canProceed}
              style={{ padding: '9px 22px', background: safeStep === totalSteps - 1 ? '#16A34A' : (!canProceed ? '#CBD5E1' : '#2563EB'), color: '#fff', border: 'none', borderRadius: 9, fontSize: 13.5, fontWeight: 600, cursor: (safeStep === totalSteps - 1 || canProceed) ? 'pointer' : 'not-allowed', fontFamily: 'inherit', boxShadow: safeStep === totalSteps - 1 ? '0 4px 14px rgba(22,163,74,.4)' : 'none' }}
            >
              {safeStep === totalSteps - 1
                ? '🚀 Launch Cycle'
                : (() => {
                    const targetIdx = canProceed ? getNextTargetIndex() : safeStep + 1;
                    return `Next: ${navSteps[targetIdx]?.label || ''} →`;
                  })()}
            </button>
          </div>
        </div>
...
export function LaunchOverview({ config, workspace, onClose }) {
  ...
  try { credentialCount = Object.keys(readEmployeeCredentialsSync() || {}).length; } catch (_) {}
...
function orgNid(node) {
  return `emp:${String(node.employee?.['Employee Code'] || '').trim().toLowerCase()}`;
}

function orgFilterNode(node, q) {
  if (!q) return node;
  const reps = (node.reports || []).map((c) => orgFilterNode(c, q)).filter(Boolean);
  const text = `${node.employee?.['Employee Name'] || ''} ${node.employee?.['Employee Code'] || ''} ${node.employee?.Designation || ''}`.toLowerCase();
...
function OrgNode({ node, depth = 0, gid = '', expandedOrgNodeIds, toggleNode, searchActive }) {
  ...
  const name = emp['Employee Name'] || emp['Employee Code'] || 'Employee';
  const code = emp['Employee Code'] || '—';
  const title = String(emp.Designation || emp.Department || emp['Group Name'] || '').trim();
  const stageId = getEmpStage(emp);

  const cardBg = '#fff';
  const cardBorder = '#E2E8F0';
  const titleColor = '#0F172A';
  const avatarBg = depth === 0 ? 'linear-gradient(135deg,#2563EB,#4F46E5)' : 'linear-gradient(135deg,#475569,#334155)';
  const badgeLabel = depth === 0 ? 'Root' : 'Employee';
  const badgeBg = depth === 0 ? '#DBEAFE' : '#F1F5F9';
  const badgeColor = depth === 0 ? '#1D4ED8' : '#64748B';

  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 236 }}>
      <div style={{
        width: 216, maxWidth: '100%',
        padding: '12px 12px 11px', borderRadius: 14, background: cardBg,
        border: `1px solid ${cardBorder}`, fontFamily: 'inherit',
        boxShadow: depth === 0 ? '0 10px 24px rgba(37,99,235,.10)' : '0 8px 22px rgba(15,23,42,.06)',
      }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
          <div style={{ display: 'flex', gap: 10, flex: 1, minWidth: 0 }}>
            <div style={{
              width: 38, height: 38, borderRadius: 12, background: avatarBg, color: '#fff',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 13, fontWeight: 800, letterSpacing: '.02em', flexShrink: 0,
            }}>
              {getInitials(name)}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13.5, fontWeight: 800, color: titleColor, lineHeight: 1.3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</div>
              <div style={{ fontSize: 11.5, color: '#64748B', marginTop: 2, lineHeight: 1.35, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {title || 'Team member'}
              </div>
            </div>
          </div>
          {expandable && (
            <button type="button" onClick={() => toggleNode(id)} aria-label={expanded ? 'Collapse' : 'Expand'}
              ...
              style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 999, cursor: 'pointer',
                padding: '2px 8px', color: '#2563EB', fontSize: 10.5, fontWeight: 700, lineHeight: 1.4, fontFamily: 'inherit',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginTop: 10, paddingLeft: 48 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', fontSize: 10.5, fontWeight: 700, color: badgeColor, background: badgeBg, borderRadius: 999, padding: '3px 8px' }}>
            {badgeLabel}
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', fontSize: 10.5, fontWeight: 700, color: '#475569', background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 999, padding: '3px 8px' }}>
            {code}
          </span>
...
function OrgNodeChildren({ kids, depth, gid, expanded, expandedOrgNodeIds, toggleNode, searchActive }) {
  ...
  const lineColor = '#D6DEEA'; const lineW = 2;
  const single = kids.length === 1; const gap = 16; const trunk = 22;

  const content = (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 'max-content', minWidth: '100%' }}>
      <div style={{ width: lineW, height: trunk, background: lineColor, flexShrink: 0, borderRadius: 999 }} />
      {single
        ...
        : <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', width: 'max-content', minWidth: '100%' }}>
            {kids.map((child, i) => {
              ...
              return (
                <div key={orgNid(child)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 'max-content', minWidth: 246, padding: `0 ${gap / 2}px` }}>
                  <div style={{ width: '100%', height: trunk, position: 'relative', flexShrink: 0 }}>
                    <div style={{ position: 'absolute', top: 0, left: isF ? '50%' : 0, right: isL ? '50%' : 0, height: lineW, background: lineColor, borderRadius: 999 }} />
                    <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: lineW, height: '100%', background: lineColor, borderRadius: 999 }} />
                  </div>
    ...
    function buildCoverage({ id, name, members, pal }) {
      ...
      members.forEach((emp) => {
        ...
        if (!memberCodeMap.has(mgrCode)) {
          if (!externalManagerMap.has(mgrCode)) {
            ...
            externalManagerMap.set(mgrCode, {
              code: mgrCodeRaw,
      ...
      const externalRoots = Array.from(externalManagerMap.entries())
        ...
        .map(([mgrCode, mgr]) => {
          const fakeEmp = { 'Employee Name': mgr.name, 'Employee Code': mgr.code };
          const reports = (directReportsMap.get(mgrCode) || [])
            .slice().sort((a, b) => String(a['Employee Name'] || '').localeCompare(String(b['Employee Name'] || '')))
            .map((e) => buildNode(e, new Set([mgrCode])));
          return { type: 'employee', employee: fakeEmp, reports, reportCount: reports.length };
        });
    ...
    <div style={{fontFamily:"'Geist','Inter','Segoe UI',Arial,sans-serif",position:'relative',minHeight:fsMode?'100vh':200,
      ...
      {!allEmpty && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
          <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155', background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 999, padding: '5px 10px' }}>Root</span>
          <span style={{ fontSize: 11.5, fontWeight: 700, color: '#6D28D9', background: '#F5F3FF', border: '1px solid #DDD6FE', borderRadius: 999, padding: '5px 10px' }}>Cross-group manager</span>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 20 }}>
          {(() => {
            const sections = populatedGroups.map((group) => {
              ...
              const roots = filtered.length > 0 ? filtered : (q ? [] : group.members.map((m) => ({ type: 'employee', employee: m, reports: [], reportCount: 0 })));
              return { group, roots };
            }).filter((section) => !q || section.roots.length > 0);
            ...
            return sections.map(({ group, roots }) => (
              <div key={group.id} style={{ borderRadius: 16, border: `1px solid ${group.pal.border}`, background: 'rgba(255,255,255,.82)', backdropFilter: 'blur(6px)', boxShadow: '0 12px 30px rgba(15,23,42,.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 16px', borderBottom: '1px solid rgba(226,232,240,.9)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                    <span style={{ width: 10, height: 10, borderRadius: '50%', background: group.pal.dot, flexShrink: 0 }} />
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 800, color: '#0F172A', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{group.name}</div>
                      <div style={{ fontSize: 11.5, color: '#64748B', marginTop: 2 }}>{group.members.length} members · {roots.length} root branch{roots.length === 1 ? '' : 'es'}</div>
                    </div>
                  </div>
                  <span style={{ fontSize: 11.5, fontWeight: 700, color: group.pal.text, background: group.pal.bg, border: `1px solid ${group.pal.border}`, borderRadius: 999, padding: '4px 10px', flexShrink: 0 }}>
                    {group.members.length}
                  </span>
                </div>
                <div style={{ overflow: 'auto', padding: '18px 24px 24px' }}>
                  <div style={{ zoom: orgZoom, display: 'flex', alignItems: 'flex-start', gap: 22, width: 'max-content', minWidth: '100%', justifyContent: roots.length === 1 ? 'center' : 'flex-start' }}>
                    {roots.map((root) => (
                      <OrgNode key={`${group.id}:${orgNid(root)}`} node={root} depth={0} gid={group.id} expandedOrgNodeIds={expandedOrgNodeIds} toggleNode={toggleNode} searchActive={!!q} />
                    ))}
                  </div>
                </div>
              </div>
                    ...
                    <div style={{ overflow: 'auto', padding: '4px 18px 24px', scrollBehavior: 'smooth' }}>
                      <div style={{ zoom: orgZoom, display: 'flex', justifyContent: 'center', width: 'max-content', minWidth: '100%', padding: '8px 0' }}>
                        {q && filtered.length === 0
                          ...
                          : <div style={{ display: 'flex', alignItems: 'flex-start', gap: 22, width: 'max-content', minWidth: '100%', justifyContent: groupRoots.length === 1 ? 'center' : 'flex-start' }}>
                              {groupRoots.map((root) => (
                                <OrgNode key={`${group.id}:${orgNid(root)}`} node={root} depth={0} gid={group.id} expandedOrgNodeIds={expandedOrgNodeIds} toggleNode={toggleNode} searchActive={!!q} />
                              ))}
                            </div>}
...
function orgMapCode(emp) {
  return String(emp?.['Employee Code'] || '').trim();
}

function orgMapName(emp) {
  return String(emp?.['Employee Name'] || emp?.['Employee Code'] || 'Employee').trim();
}

function orgMapGroup(emp) {
  return String(emp?.assignedGoalGroupName || emp?.['Group Name'] || '').trim();
}

function orgMapField(emp, keys) {
  const source = emp || {};
  for (const key of keys) {
    const direct = source[key];
    if (direct !== undefined && direct !== null && String(direct).trim()) return String(direct).trim();
    const normalized = normalizeEmployeeFieldKey(key);
    const found = Object.keys(source).find((item) => normalizeEmployeeFieldKey(item) === normalized);
    if (found && source[found] !== undefined && source[found] !== null && String(source[found]).trim()) {
      return String(source[found]).trim();
    }
  }
  return '';
}

function orgMapNumber(emp, keys) {
  const raw = orgMapField(emp, keys).replace('%', '');
  const parsed = Number(raw);
  return Number.isFinite(parsed) ? parsed : null;
}

function orgMapStageMeta(emp) {
...
function orgMapGoalPct(emp) {
  const explicit = orgMapNumber(emp, ['Goal Completion %', 'Goal Completion', 'Goal %', 'Goals %', 'Completion %', 'Progress %']);
...
function orgMapDateValue(emp, keys) {
  const raw = orgMapField(emp, keys);
  if (!raw) return null;
  const parsed = new Date(raw);
  return Number.isNaN(parsed.getTime()) ? null : parsed;
}

function orgMapDateLabel(date) {
  if (!date) return 'Not logged';
  return date.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
}

function orgMapRating(emp) {
  return orgMapNumber(emp, ['Rating', 'Final Rating', 'Manager Rating', 'Performance Rating']);
}

function orgMapFlags(emp) {
  const stage = getEmpStage(emp);
  const goalPct = orgMapGoalPct(emp);
  const joinDate = orgMapDateValue(emp, ['Joining Date', 'Date of Joining', 'Join Date', 'Start Date']);
  const daysSinceJoin = joinDate ? Math.floor((Date.now() - joinDate.getTime()) / 86400000) : null;
  const pip = /^(yes|y|true|1|pip|on pip)$/i.test(orgMapField(emp, ['PIP', 'On PIP', 'Performance Improvement Plan']));
  const goalsOverdue = /overdue/i.test(orgMapField(emp, ['Goal Status', 'Goals Status'])) || (goalPct < 100 && (stage === 'goal-creation' || stage === 'pending-approval'));
  const reviewOverdue = /overdue/i.test(orgMapField(emp, ['Review Status', 'Review Overdue'])) || stage === 'self-evaluation' || stage === 'mgr-evaluation';
  return {
    noGoals: goalPct === 0,
    goalsOverdue,
    reviewOverdue,
    newHire: daysSinceJoin !== null && daysSinceJoin >= 0 && daysSinceJoin <= 90,
    pip,
    attention: goalsOverdue || reviewOverdue || pip,
  };
}

function orgMapNodeStats(node) {
  const reports = [];
  function collect(current) {
    (current?.reports || []).forEach((child) => {
      reports.push(child.employee);
      collect(child);
    });
  }
  collect(node);
  const ratings = reports.map(orgMapRating).filter((value) => value !== null);
  const avg = ratings.length ? (ratings.reduce((sum, value) => sum + value, 0) / ratings.length).toFixed(1) : '—';
  return {
    reports: reports.length,
    goalsPending: reports.filter((emp) => orgMapGoalPct(emp) < 100).length,
    reviewOverdue: reports.filter((emp) => orgMapFlags(emp).reviewOverdue).length,
    avg,
  };
}

function orgMapCardMeta(emp, group) {
  const role = orgMapField(emp, ['Designation', 'Role', 'Job Title', 'Title']);
  const level = orgMapField(emp, ['Level', 'Grade', 'Band']);
  const configuredAttr = String(group?.segmentAttr || group?.groupAttr || '').trim();
  const configuredValue = configuredAttr ? orgMapField(emp, [configuredAttr]) : '';
  const fallbackAttr = configuredAttr ? '' : ['Department', 'Function', 'Business Unit', 'Grade', 'Band', 'Location'].find((key) => orgMapField(emp, [key]));
  const attrLabel = configuredAttr || fallbackAttr || '';
  const attrValue = configuredValue || (fallbackAttr ? orgMapField(emp, [fallbackAttr]) : '');
  return {
    roleLine: [role, level].filter(Boolean).join(' · '),
    attrLabel,
    attrValue,
  };
}
...
function buildOrgMapGroups(employees, groups) {
  ...
  const knownNames = new Set((groups || []).map((g) => String(g?.name || '').trim().toLowerCase()).filter(Boolean));
  const baseGroups = (groups || []).map((group, gi) => {
    const name = String(group?.name || `Group ${gi + 1}`).trim();
  ...
  function buildCoverage(group) {
    const memberByCode = new Map(group.members.map((emp) => [orgMapCode(emp).toLowerCase(), emp]));
    ...
    group.members.forEach((emp) => {
      const managerCodeRaw = String(emp['Reporting Manager Code'] || '').trim();
      const managerCode = managerCodeRaw.toLowerCase();
      if (!managerCode) {
        noManagerCount += 1;
        return;
      }
      const list = reportsByManager.get(managerCode) || [];
      list.push(emp);
      reportsByManager.set(managerCode, list);
      ...
      if (!memberByCode.has(managerCode)) {
        ...
        if (managerEmp?._outsidePms) {
          ...
          externalManagers.set(managerCode, {
            'Employee Code': managerCodeRaw,
            'Employee Name': managerName || managerCodeRaw,
            _externalManager: true,
          });
        }
      }
    });
    ...
    function buildNode(emp, trail = new Set()) {
      const code = orgMapCode(emp).toLowerCase();
      if (!code || trail.has(code)) return { employee: emp, reports: [], reportCount: 0 };
      const nextTrail = new Set(trail);
      nextTrail.add(code);
      const reports = (reportsByManager.get(code) || [])
        .slice()
        .sort((a, b) => orgMapName(a).localeCompare(orgMapName(b)))
        .map((child) => buildNode(child, nextTrail));
      return { employee: emp, reports, reportCount: reports.length };
    }
    ...
    externalManagers.forEach((managerEmp, managerCode) => {
      const reports = (reportsByManager.get(managerCode) || [])
        .slice()
        .sort((a, b) => orgMapName(a).localeCompare(orgMapName(b)))
        .map((emp) => buildNode(emp, new Set([managerCode])));
      roots.push({ employee: managerEmp, reports, reportCount: reports.length, externalRoot: true });
    });

    crossGroupManagers.forEach((managerEmp, managerCode) => {
      const reports = (reportsByManager.get(managerCode) || [])
        .slice()
        .sort((a, b) => orgMapName(a).localeCompare(orgMapName(b)))
        .map((emp) => buildNode(emp, new Set([managerCode])));
  ...
  return baseGroups.map(buildCoverage).filter((group) => group.members.length > 0);
}

function OrgMapCard({ node, selectedCode, onSelect, compact = false, viewMode = 'org', group = null }) {
  const emp = node.employee || {};
  const code = orgMapCode(emp);
  const name = orgMapName(emp);
  const selected = code && code.toLowerCase() === selectedCode;
  // RM-not-in-PMS is intentionally NOT typecast as external — it can be a late
  ...
  return (
    <button type="button" onClick={() => onSelect(emp)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        <div style={{ width: compact ? 30 : 38, height: compact ? 30 : 38, borderRadius: 10, background: avatarBg, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: compact ? 10 : 12, fontWeight: 800, flexShrink: 0 }}>
          {getInitials(name)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: compact ? 12.2 : 13.2, fontWeight: 800, color: '#0F172A', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</div>
          <div style={{ fontSize: 11.2, color: '#64748B', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      <div style={{ marginTop: 9, paddingLeft: compact ? 40 : 48 }}>
        ...
        {!isExternal && !isCross && !isOutsidePms && (
          <div style={{ marginTop: meta.roleLine || (meta.attrLabel && meta.attrValue) ? 8 : 0 }}>
            ...
            <div style={{ marginTop: 4, height: 5, borderRadius: 999, background: '#E2E8F0', overflow: 'hidden' }}>
              <div style={{ width: `${goalPct}%`, height: '100%', borderRadius: 999, background: goalPct >= 100 ? '#16A34A' : goalPct > 50 ? '#4F46E5' : '#D97706' }} />
            </div>
          </div>
        )}
      </div>
...
function OrgMapExpandButton({ open, count, onClick, compact = false }) {
  const label = open
    ? 'Hide reports'
    : `Show ${count} report${count === 1 ? '' : 's'}`;
  return (
    <button type="button" onClick={onClick} aria-expanded={open}
      style={{
        ...
        cursor: 'pointer',
        fontFamily: 'inherit',
        boxShadow: open ? 'none' : '0 8px 18px rgba(15,23,42,.06)',
      }}>
      <span style={{
        width: compact ? 17 : 19,
        height: compact ? 17 : 19,
        borderRadius: '50%',
        background: open ? '#E2E8F0' : '#EEF2FF',
        color: open ? '#475569' : '#4F46E5',
        display: 'inline-flex',
        alignItems: 'center',
        justifyContent: 'center',
        fontSize: 12,
...
function OrgMapBranch({ node, selectedCode, onSelect, defaultOpen = false, viewMode = 'org', group = null }) {
  const [open, setOpen] = useState(defaultOpen);
  const children = node.reports || [];
  const visibleChildren = open ? children : children.slice(0, 4);
  const hiddenCount = Math.max(0, children.length - visibleChildren.length);

  return (
    <div style={{ minWidth: 255, maxWidth: 310, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <OrgMapCard node={node} selectedCode={selectedCode} onSelect={onSelect} viewMode={viewMode} group={group} />
      {children.length > 0 && (
        <div style={{ borderLeft: '1px solid rgba(148,163,184,.38)', marginLeft: 18, paddingLeft: 14, display: 'flex', flexDirection: 'column', gap: 9 }}>
          {visibleChildren.map((child) => (
            <OrgMapNestedNode key={orgMapCode(child.employee) || orgMapName(child.employee)} node={child} selectedCode={selectedCode} onSelect={onSelect} viewMode={viewMode} group={group} />
          ))}
          {children.length > 4 && (
            <OrgMapExpandButton open={open} count={hiddenCount || children.length} onClick={() => setOpen((value) => !value)} />
          )}
        </div>
      )}
    </div>
...
function OrgMapNestedNode({ node, selectedCode, onSelect, viewMode = 'org', group = null }) {
  const hasChildren = node.reportCount > 0;
  const [open, setOpen] = useState(false);
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        <OrgMapCard node={node} selectedCode={selectedCode} onSelect={onSelect} compact viewMode={viewMode} group={group} />
        {hasChildren && (
          <div style={{ marginLeft: 40, marginTop: -1 }}>
            <OrgMapExpandButton open={open} count={node.reportCount} onClick={() => setOpen((v) => !v)} compact />
          </div>
        )}
      </div>
      {hasChildren && open && (
        <div style={{ borderLeft: '1px solid rgba(148,163,184,.35)', marginLeft: 16, paddingLeft: 12, display: 'flex', flexDirection: 'column', gap: 7 }}>
...
export function OrgChartPanel({ employees, groups }) {
  const orgGroups = useMemo(() => buildOrgMapGroups(employees, groups), [employees, groups]);
  ...
  useEffect(() => {
    if (!orgGroups.length) return;
    setActiveGroupId((current) => orgGroups.some((group) => group.id === current) ? current : orgGroups[0].id);
  }, [orgGroups]);

  const activeGroup = orgGroups.find((group) => group.id === activeGroupId) || orgGroups[0] || null;
  const selectedCode = orgMapCode(selectedEmp).toLowerCase();
  ...
  function nodeMatches(node) {
    ...
    if (filterSet.has('stage-not-started') && stage.key !== 'not-started') return false;
    if (filterSet.has('stage-self-review') && stage.key !== 'self-review') return false;
    if (filterSet.has('stage-manager-review') && stage.key !== 'manager-review') return false;
    if (filterSet.has('stage-calibration') && stage.key !== 'calibration') return false;
    if (filterSet.has('stage-closed') && stage.key !== 'closed') return false;
    return true;
  }

  function nodeVisible(node) {
    return nodeMatches(node) || (node.reports || []).some(nodeVisible);
  }

  const visibleRoots = useMemo(() => {
    if (!activeGroup) return [];
    return (query || activeFilters.length) ? activeGroup.roots.filter(nodeVisible) : activeGroup.roots;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeGroup, query, activeFilters]);
  ...
  const filterChips = [
    { key: 'no-goals', label: 'No goals set', title: 'Employees with 0% goal completion or still at the start of goal setup.' },
    { key: 'goals-overdue', label: 'Goals overdue', title: 'Employees whose goals appear incomplete or overdue.' },
  ];
  const selectedMeta = orgMapCardMeta(selectedEmp, activeGroup);
  const selectedDetailRows = [
    ['Manager', selectedEmp?.['Reporting Manager Name'] || selectedEmp?.['Reporting Manager Code'] || 'No manager'],
    selectedMeta.roleLine ? ['Role', selectedMeta.roleLine] : null,
    selectedMeta.attrLabel && selectedMeta.attrValue ? [selectedMeta.attrLabel, selectedMeta.attrValue] : null,
    ['Email', selectedEmp?.['Email ID'] || selectedEmp?.Email || 'Not set'],
  ].filter(Boolean);

  return (
    <div style={{ height: '100%', position: 'relative', display: 'flex', flexDirection: 'column', background: '#F8FAFC', fontFamily: "'Geist','Inter','Segoe UI',Arial,sans-serif", overflow: 'hidden' }}>
      <div style={{ flexShrink: 0, padding: '14px 24px 12px', background: 'rgba(255,255,255,.88)', borderBottom: '1px solid #E2E8F0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto 320px', gap: 16, alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', overflowX: 'auto', paddingBottom: 2 }}>
            {orgGroups.map((group) => {
              const active = group.id === activeGroup?.id;
              return (
                <button key={group.id} type="button" title="Show employees assigned to this PMS goal group." onClick={() => { setActiveGroupId(group.id); setSelectedEmp(null); }}
                  style={{
                    flex: '0 0 auto', display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${active ? group.pal.dot : '#E2E8F0'}`,
                    background: active ? group.pal.bg : '#fff', color: active ? group.pal.text : '#334155', borderRadius: 999,
                    padding: '8px 12px', cursor: 'pointer', fontFamily: 'inherit', boxShadow: active ? '0 10px 24px rgba(79,70,229,.08)' : 'none',
                  }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: group.pal.dot }} />
                  <span style={{ fontSize: 12.5, fontWeight: 850 }}>{group.name}</span>
                  <span style={{ fontSize: 11.5, fontWeight: 850, opacity: .75 }}>{group.members.length}</span>
                </button>
              );
            })}
          </div>
          <div style={{ display: 'inline-flex', background: '#F1F5F9', border: '1px solid #E2E8F0', borderRadius: 999, padding: 3 }}>
            {[
              { key: 'org', label: 'Org' },
              { key: 'heatmap', label: 'Heatmap' },
            ].map((item) => (
              <button key={item.key} type="button" onClick={() => setViewMode(item.key)}
                style={{ border: 'none', background: viewMode === item.key ? '#fff' : 'transparent', color: viewMode === item.key ? '#0F172A' : '#64748B', borderRadius: 999, padding: '6px 11px', fontSize: 12, fontWeight: 850, cursor: 'pointer', fontFamily: 'inherit', boxShadow: viewMode === item.key ? '0 1px 4px rgba(15,23,42,.10)' : 'none' }}>
                {item.label}
              </button>
            ))}
          </div>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search this group..."
            style={{ width: '100%', border: '1.5px solid #E2E8F0', borderRadius: 12, padding: '10px 12px', fontSize: 13, fontFamily: 'inherit', outline: 'none', color: '#0F172A', background: '#fff' }} />
        </div>
        <div style={{ marginTop: 10, display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
            <span style={{ width: 10, height: 10, borderRadius: '50%', background: activeGroup?.pal.dot || '#64748B' }} />
            <h2 style={{ margin: 0, fontSize: 18, color: '#0F172A', letterSpacing: '-.02em', whiteSpace: 'nowrap' }}>{activeGroup?.name || 'Organogram'}</h2>
            <span style={{ fontSize: 12.5, color: '#64748B', whiteSpace: 'nowrap' }}>
              {activeGroup?.members.length || 0} members · {activeGroup?.roots.length || 0} manager lanes
            </span>
          </div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto' }}>
            {signals.map((item) => (
              <button key={item.label} type="button" title={item.title} onClick={() => toggleFilter(item.key)}
                style={{ flex: '0 0 auto', display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 999, border: `1px solid ${filterSet.has(item.key) ? item.color : item.border}`, background: filterSet.has(item.key) ? item.color : item.bg, color: filterSet.has(item.key) ? '#fff' : item.color, fontSize: 11.5, fontWeight: 850, cursor: 'pointer', fontFamily: 'inherit' }}>
                {item.label}
        ...
        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8, overflowX: 'auto' }}>
          {filterChips.map((item) => {
            const active = filterSet.has(item.key);
            return (
              <button key={item.key} type="button" title={item.title} onClick={() => toggleFilter(item.key)}
                style={{ flex: '0 0 auto', border: `1px solid ${active ? '#0F172A' : '#E2E8F0'}`, background: active ? '#0F172A' : '#fff', color: active ? '#fff' : '#475569', borderRadius: 999, padding: '6px 10px', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
                {item.label}
              </button>
            );
          })}
          {(activeFilters.length > 0 || search) && (
            <button type="button" onClick={clearFilters}
              style={{ flex: '0 0 auto', border: 'none', background: 'transparent', color: '#64748B', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', padding: '6px 4px' }}>
              Clear
            </button>
          )}
        </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '28px 30px 44px', background: 'radial-gradient(circle at 18% 12%, rgba(79,70,229,.055), transparent 30%), radial-gradient(circle at 78% 18%, rgba(14,165,233,.05), transparent 28%), linear-gradient(180deg,#F8FAFC,#F9FBFF)' }}>
        {visibleRoots.length === 0 ? (
          <div style={{ padding: 22, border: '1px dashed #CBD5E1', borderRadius: 14, color: '#64748B', background: '#fff', fontSize: 13 }}>No matching employees in this group.</div>
        ) : (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18, minWidth: 'max-content', paddingBottom: 24 }}>
            {visibleRoots.map((root) => (
              <OrgMapBranch key={`${activeGroup?.id}:${orgMapCode(root.employee) || orgMapName(root.employee)}`} node={root} selectedCode={selectedCode} onSelect={setSelectedEmp} defaultOpen={visibleRoots.length <= 3} viewMode={viewMode} group={activeGroup} />
            ))}
          </div>
        )}
      </div>
      ...
      {selectedEmp && (
        <aside style={{ position: 'absolute', right: 24, bottom: 20, width: 360, maxWidth: 'calc(100vw - 48px)', border: '1px solid rgba(226,232,240,.95)', background: 'rgba(255,255,255,.96)', borderRadius: 16, padding: 14, boxShadow: '0 20px 50px rgba(15,23,42,.14)', zIndex: 5, backdropFilter: 'blur(12px)' }}>
          <div style={{ display: 'flex', gap: 11, alignItems: 'flex-start' }}>
            <div style={{ width: 40, height: 40, borderRadius: 13, background: 'linear-gradient(135deg,#2563EB,#4F46E5)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, flexShrink: 0 }}>{getInitials(orgMapName(selectedEmp))}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14.5, fontWeight: 900, color: '#0F172A', lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{orgMapName(selectedEmp)}</div>
                  <div style={{ marginTop: 3, fontSize: 12, color: '#64748B' }}>{orgMapCode(selectedEmp) || 'No employee code'} · {orgMapGroup(selectedEmp) || 'Unassigned'}</div>
                </div>
                <button type="button" onClick={() => setSelectedEmp(null)}
                  style={{ width: 28, height: 28, borderRadius: 10, border: '1px solid #E2E8F0', background: '#fff', color: '#64748B', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}
              ...
              <div style={{ marginTop: 11, display: 'grid', gap: 7 }}>
                {selectedDetailRows.map(([label, value]) => (
                  <div key={label} style={{ display: 'grid', gridTemplateColumns: '68px minmax(0,1fr)', gap: 8, alignItems: 'baseline' }}>
                    <div style={{ fontSize: 10.5, fontWeight: 850, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '.04em' }}>{label}</div>
                    <div style={{ fontSize: 12.2, fontWeight: 750, color: '#0F172A', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</div>
                  </div>
                ))}
              </div>
            </div>
