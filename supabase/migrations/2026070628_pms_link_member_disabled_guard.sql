-- Guard: link_invited_member_tx must never downgrade an already-active member
-- back to 'invited'. Re-inviting a participant who has already accepted (status
-- 'active') would otherwise lock them out. The upsert now only flips to 'invited'
-- when the member is not already active, and re-fetches the id in the no-op case.
...
begin
  ...
  on conflict (organization_id, user_id) do update set status = 'invited'
    where pms.org_members.status not in ('active','disabled')
  returning id into v_member_id;
  -- ON CONFLICT ... WHERE false performs no update and returns nothing, so
  -- re-fetch the member id for an already-active member (kept 'active').
  if v_member_id is null then
    select id into v_member_id from pms.org_members
